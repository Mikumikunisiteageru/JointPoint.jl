...
@testset "findjoint" begin
	...
	@test xj0 == [0, 10]
	@test isapprox(yj0, a .* [0, 10] .+ b; atol=1	e-6)
	...
	@test isapprox(yj1k, Float32[36.417873, 11.273728, 0.0866707, -0.042032868]; rtol=1e-2)
	f(k) = aicc(xc1k, yc1k, findjoint(xc1k, yc1k, k; TraceMode=:silent)...)
	aiccs = f.(1:5)
	@test isapprox(aiccs, Float32[331.5818, -1081.6448, -1184.8989, -1185.5844, -1182.1051])
end
